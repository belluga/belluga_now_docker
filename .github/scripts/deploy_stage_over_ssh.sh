...
run_migrations() {
  ...
  local tenant_count_raw tenant_count
  if ! tenant_count_raw="\$(
    "\${DOCKER_COMPOSE[@]}" exec -T app php -r \
    ...
    echo "ERROR: unable to resolve tenant count; refusing to skip tenant migrations." >&2
    return 1
  fi
  tenant_count="\$(printf '%s' "\${tenant_count_raw}" | awk '{\$1=\$1; print}')"

  if [[ ! "\${tenant_count}" =~ ^[0-9]+$ ]]; then
    echo "ERROR: invalid tenant count; refusing to skip tenant migrations." >&2
    return 1
  fi
